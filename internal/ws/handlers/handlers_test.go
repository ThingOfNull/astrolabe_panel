...
func TestPingHandler(t *testing.T) {
	...
	server := ws.NewServer(reg)
	out, err := server.DispatchRaw(context.Background(), []byte(`{"jsonrpc":"2.0","id":1,"method":"ping"}`))
	if err != nil {
...
func TestBoardGetHandler(t *testing.T) {
	...
	out, err := server.DispatchRaw(context.Background(), []byte(`{"jsonrpc":"2.0","id":42,"method":"board.get"}`))
	if err != nil {
		t.Fatalf("DispatchRaw: %v", err)
	}
	if out == nil {
