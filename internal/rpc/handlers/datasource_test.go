...
func setupDS(t *testing.T) (*rpc.Registry, *datasource.Manager, *store.Store) {
	t.Helper()
	dir := t.TempDir()
	...
	reg := rpc.NewRegistry()
	RegisterDataSource(reg, s, mgr, nil)
	return reg, mgr, s
}
...
func TestDataSourceCRUD(t *testing.T) {
	...
	out, _ := rpc.DispatchRaw(ctx, reg, []byte(`{"jsonrpc":"2.0","id":1,"method":"datasource.types"}`))
	...
	out, _ = rpc.DispatchRaw(ctx, reg, create)
	...
	out, _ = rpc.DispatchRaw(ctx, reg, []byte(`{"jsonrpc":"2.0","id":3,"method":"datasource.list"}`))
	var listResp struct {
		Result struct {
	...
	out, _ = rpc.DispatchRaw(ctx, reg, test)
	var testResp struct {
	...
	discover := []byte(`{"jsonrpc":"2.0","id":5,"method":"datasource.discover","params":{"id":` +
		jsonNumber(id) + `}}`)
	out, _ = rpc.DispatchRaw(ctx, reg, discover)
